...
class ConversationV1 {
    ...
    options) {
        ...
        return new PreparedMessage(env, async () => {
            ...
            return DecodedMessage.fromV1Message(msg, content, options?.contentType || contentTypeText.ContentTypeText, payload, topic, this);
        });
    }
    /**
    ...
    async send(content, options) {
        ...
        const contentType = options?.contentType || contentTypeText.ContentTypeText;
        const { payload } = await this.client.encodeContent(content, options);
        const msg = await this.createMessage(payload, recipient, options?.timestamp);
        await this.client.publishEnvelopes(topics.map((topic) => ({
            contentTopic: topic,
...
class ConversationV2 {
    ...
    options) {
        ...
        return new PreparedMessage(env, async () => {
            ...
            return DecodedMessage.fromV2Message(msg, content, options?.contentType || contentTypeText.ContentTypeText, topic, payload, this, this.client.address);
        });
    }
...
function defaultOptions(opts) {
    const _defaultOptions = {
        ...
        env: 'dev',
        apiUrl: undefined,
        codecs: [new contentTypeText.TextCodec()],
...
class Client {
    ...
    async decodeContent(contentBytes) {
        ...
        const contentType = new contentTypePrimitives.ContentTypeId(encodedContent.type);
        let error;
