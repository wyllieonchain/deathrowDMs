import { ciphertext, privateKey, signature, publicKey, fetcher, keystore, message, content, authn, messageApi, invitation, contact, privatePreferences } from '@xmtp/proto';
import Long from 'long';
import * as secp from '@noble/secp256k1';
import { bytesToHex as bytesToHex$1, keccak256, getAddress, hexToSignature, hexToBytes, hashMessage, verifyMessage } from 'viem';
import { webcrypto } from 'crypto';
import { ContentTypeText, TextCodec } from '@xmtp/content-type-text';
import { ContentTypeId } from '@xmtp/content-type-primitives';
import { Mutex } from 'async-mutex';
import elliptic from 'elliptic';
import { user_preferences_encrypt, user_preferences_decrypt, generate_private_preferences_topic } from '@xmtp/user-preferences-bindings-wasm';
...
function computeAddress(bytes) {
    const publicKey = bytesToHex$1(bytes.slice(1));
...
class WalletSigner {
    ...
    static signerKey(key, signature) {
        const digest = hexToBytes(hashMessage(this.identitySigRequestText(key.bytesToSign())));
        return ecdsaSignerKey(digest, signature);
    }
    async signKey(key) {
...
class PublicKey extends UnsignedPublicKey {
    ...
    walletSignatureAddress() {
        ...
        const digest = hexToBytes(hashMessage(WalletSigner.identitySigRequestText(this.bytesToSign())));
...
class MessageV1 extends MessageBase {
    ...
    static fromBytes(bytes) {
        const message$1 = message.Message.decode(bytes);
...
class JobRunner {
    ...
    async getLastRunTime() {
        const { lastRunNs } = await this.keystore.getRefreshJob(keystore.GetRefreshJobRequest.fromPartial({
            jobType: this.protoJobType,
        }));
...
const { GetKeystoreStatusResponse_KeystoreStatus: KeystoreStatus$1, InitKeystoreRequest, InitKeystoreResponse, GetKeystoreStatusRequest, GetKeystoreStatusResponse, } = keystore;
    ...
    async decodeMessages(messages) {
        ...
        const actions = responses.reduce((result, response) => {
            return response.result?.decrypted
                ? result.concat(privatePreferences.PrivatePreferencesAction.decode(response.result.decrypted))
                : result;
        }, []);
        return actions;
    }
    ...
    async publish(entries) {
        ...
        const actions = entries.reduce((result, entry) => {
            ...
            if (entry.entryType === 'address') {
                ...
                return result.concat(privatePreferences.PrivatePreferencesAction.encode(action).finish());
            }
            return result;
        }, []);
...
class Client {
    ...
    async getUserContacts(peerAddresses) {
        ...
        const normalizedAddresses = peerAddresses.map((address) => getAddress(address));
    ...
    static async canMessage(peerAddress, opts) {
        ...
        if (Array.isArray(peerAddress)) {
            ...
            const normalizedPeerAddresses = rawPeerAddresses.map((address) => getAddress(address));
    ...
    async encodeContent(content$1, options) {
        ...
        return {
            payload: content.EncodedContent.encode(encoded).finish(),
...
export { ApiUrls, AuthCache, BrowserStoragePersistence, Ciphertext, Client, Compression, ConsentListEntry, ConversationV1, ConversationV2, Conversations, DecodedMessage, EncryptedPersistence, HttpApiClient, InMemoryKeystore, InMemoryPersistence, InvitationV1, KeyGeneratorKeystoreProvider, LocalAuthenticator, MessageV1, MessageV2, NetworkKeystoreProvider, PrefixedPersistence, PrivateKey, PrivateKeyBundleV1, PrivateKeyBundleV2, PublicKey, PublicKeyBundle, SealedInvitation, Signature, SignedPublicKey, SignedPublicKeyBundle, SnapKeystoreProvider as SnapProvider, SortDirection, StaticKeystoreProvider, Stream, buildContentTopic, buildDirectMessageTopic, buildDirectMessageTopicV2, buildUserContactTopic, buildUserIntroTopic, buildUserInviteTopic, buildUserPrivateStoreTopic, dateToNs, decodeContactBundle, decodeContent, decrypt$1 as decrypt, defaultKeystoreProviders, encrypt$1 as encrypt, exportHmacKey, fromNanoString, generateHmacSignature, hkdfHmacKey, importHmacKey, apiDefs as keystoreApiDefs, mapPaginatedStream, nsToDate, retry, snapApiDefs as snapKeystoreApiDefs, toNanoString, verifyHmacSignature };
